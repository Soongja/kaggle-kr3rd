...
def load_checkpoint(config, model, checkpoint):
    ...
    return last_epoch, score


def save_checkpoint(config, model, epoch, score, weights_dict=None):
    checkpoint_dir = os.path.join(config.TRAIN_DIR, 'checkpoints')

    checkpoint_path = os.path.join(checkpoint_dir, 'epoch_%04d_%.4f.pth' % (epoch, score))
